...
!!! success "1. Best model: 92.7% on eval2, 4.5 points behind Jev"
    **Qwen3-4B-Instruct-2507 + LoRA r=64, shared-prefix tree, round-2 data** (`tree_4b_instruct_r2x64`) scores
...
!!! warning "3. The dev benchmark hid most real effects; eval2 exposed them"
    Every model, frontier ones included, sits between 80% and 86% on the dev benchmark: Jev 82.7, GPT-6 Astra 85.8.
    ...
    On eval2 the same models spread from 68.8% to 92.7%, and Jev scores 97.2%. Levers the dev benchmark called ties or
...
!!! info "4. eval2: 1,991 questions, three authors, two blind judges, $32.72"
    ...
    - Balanced by construction: 3 difficulty tiers, a text-length ladder from 8 to 8K tokens, every trap tag with
      n ≥ 57 (except `evidence_start`, 17).
...
!!! success "6. Verified target-task data is the biggest lever: +5.5 on eval2"
    Round 2 added about 10K hard-case questions (six authoring models, blind GPT-6 Astra judge). Tree 4B, round 1 vs
...
!!! success "10. The shared-prefix tree: same quality, up to 37× faster"
    One forward pass over a token tree: the text is read once, and every question and candidate branches off it with
    full attention to it through all layers. Each leaf scores exactly like the standalone pair (tested to 1.7e-5).
...
!!! tip "11. The base model matters more than its size"
    - **Instruct beats reranker once the data is good:** Qwen3-4B-Instruct-2507 in the tree scores +3.0 on eval2 with
      round-1 data (88.1 vs 85.1) and stacks with round-2 data (92.7 vs 90.6, 90 / 47, p = 0.0003). On the dev
      benchmark it looked like a loss (80.4 vs 81.6).
    - **4B ≈ 8B:** stock + LoRA 80.3% vs 80.7% on the dev benchmark (p = 0.52), and the 8B is 1.7× slower.
...
!!! info "16. A 27B zero-shot teacher is as good as our best 4B, and its errors differ"
...
!!! success "20. Serving: vLLM and merged weights are the useful speed-ups"
    ...
    - Merging the LoRA into the weights: 15–35% faster; bf16 costs no quality (0.6B: 73.7 vs 73.5).
...
!!! info "21. A busy GPU is cheaper than Jev; an idle one is not"
    - Fully busy, one A10G with vLLM costs $0.005–0.27 per 1,000 requests; Jev charges $0.016–0.27 ($0.042 per
      million input tokens). That is 1.3–3× cheaper, equal at 4,096 tokens × 16 questions.
...
!!! abstract "22. Multilabel, numbers, dates, sarcasm and injections"
    Best model (`tree_4b_instruct_r2x64`) vs Jev on eval2, largest gaps first (trap slices with n ≥ 100):

...
For every (state, question, candidate) triple:
...
| `data/synthetic.jsonl` | hard, long, trap-heavy **training** data in 6 families | 924 states / 2,448 questions: train 2,112, validation 115, calibration 125, test 96 | written by 12 Claude Sonnet agents from [data/synthetic/BRIEF.md](data/synthetic/BRIEF.md); labels are LLM-intended, not ground truth; generation stopped at about 75% of the plan when a usage limit hit |
...
## Next: Qwen3-Reranker-4B

The code is model-agnostic: `Scorer(model_id=…, revision=…)`. Checked on 2026-09-23, `Qwen/Qwen3-Reranker-4B`
at revision `22e683669bc0f0bd69640a1354a6d0aebcfeede5` has a byte-identical chat template and the same yes/no
token ids (9693/2152); the Scorer re-checks the ids on load. The 4B model has hidden size 2560 and 36 layers.
The plan:

1. Pin that revision.
2. Rerun baseline → LoRA → external comparison on the same splits and failure file.
3. Grow the training set specifically for the tags in `failures.md` with *new* examples.

At about 7× the FLOPs, 4B inference in bf16 would be ~1 s for the smallest request here on this Mac, and
LoRA training ~7 h per epoch, so a rented GPU is the practical path. Provisioning a paid machine waits for
an explicit go-ahead with its hourly price.

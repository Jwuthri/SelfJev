...
**Start here across sessions:** [docs/experiments.md](docs/experiments.md) is the ledger: every measured model
(generated by `scripts/ledger.py`), conclusions, dead ends, known issues and open ideas to claim. Round-2 hard-case
data, blind judging and the retrain diagnosis: [docs/hardcases_round2.md](docs/hardcases_round2.md). A 0.6B listwise
alternative backbone (jina-reranker-v3.5) and why it falls short: [docs/jina_model.md](docs/jina_model.md).
...
| tree scorer with more LoRA capacity: rank 64 (4× the trainable parameters), or rank 16 on attention + MLP matrices (2.8×) | old test | 81.5 / 81.6 vs 81.6 for rank 16 on attention (p = 0.94 / 1.0) |
| | eval2 (1,991 target-task questions, `reports/eval2/summary.md`) | **87.2 / 86.3 vs 85.1** (p = 0.00014 / 0.021) |
| the two together: verified round-2b data + rank 64, and + MLP matrices | eval2 | 90.3 / **91.3** vs 90.6 for round-2b at rank 16 (p = 0.61 / 0.17); old test 81.2 / 82.4 vs 81.2 |
...
- More adapter capacity on the tree scorer raises validation accuracy (80.2 → 81.8), leaves the old test unchanged and
  gains 1.2–2.1 points on eval2: the old test, dominated by public held-out families, cannot see it.
- On the old test, data volume, epochs, base model and adapter capacity all land at 80–82%. On eval2 the verified
  round-2 data (+5.5) and adapter capacity (+2.1) both count but overlap: combined they reach 91.3, and Jev leads by
  about 6 points (97.2).
...
| `data/eval2.jsonl` | **frozen target-task test set** (round 2): 3 difficulty tiers × 10 length buckets (8–8K tokens), every trap tag with n ≥ 57, `none` correct 14.8% when offered | 1,991 questions / 647 states, all `test` | written by Claude Opus 5.5, Kimi K3 and GLM 5.3 (no training-data author; `scripts/gen_hardcases.py --eval2`), kept only where two blind judges (GPT-6 Astra, Gemini 3.1 Pro) agree with the author (`scripts/judge_hardcases.py`, `scripts/build_eval2.py`); Jev scores 97.2% on it; **never train, select prompts or fit calibration on it**; review [data/eval2/REVIEW.md](data/eval2/REVIEW.md) |
| `data/hardcases.jsonl` | round-2 hard-case **training** data: 3 difficulty tiers × 10 length buckets (8–8K tokens), 6 authors | 10,142 questions kept of 10,627 (train 0.9 / validation 0.1) | written by Gemini 3.8 Flash, Grok 4.7, GPT-6 Luna, DeepSeek V4 Flash via OpenRouter ([scripts/gen_hardcases.py](scripts/gen_hardcases.py), brief [data/hardcases/BRIEF.md](data/hardcases/BRIEF.md)) and 8 Claude Sonnet agents; every question re-labelled blind by GPT-6 Astra ([scripts/judge_hardcases.py](scripts/judge_hardcases.py)), disagreements dropped; see [docs/hardcases_round2.md](docs/hardcases_round2.md) |

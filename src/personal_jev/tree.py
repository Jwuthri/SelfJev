...
def format_config(model_id: str) -> dict:
...
class Encoder:
    ...
    def __init__(self, tokenizer, model_id):
        self.tok = tokenizer
        t = lambda s: tokenizer(s, add_special_tokens=False)["input_ids"]
        self.prefix = t(PREFIX)
        self.state_pre, self.state_post = t(STATE_TEXT[0]), t(STATE_TEXT[1])
        self.q_pre, self.q_post = t(QUESTION_TEXT[0]), t(QUESTION_TEXT[1])
        self.leaf_pre, self.suffix = t(LEAF_TEXT), t(suffix_for(model_id))
...
def tree_mask(tree) -> torch.Tensor:
...
class TreeModel:
    ...
    def cached(self, trees):
        ...
        for b, t in enumerate(trees):
            ...
            m = tree_mask(t)
            allowed[b, :n, :r] = m[r:, :r]  # the whole (real) root; its padding columns stay masked
            allowed[b, :n, R:R + n] = m[r:, r:]

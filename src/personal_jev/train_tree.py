...
def encode_items(enc, examples, max_length):
    ...
    for (ex, q, s), qi in zip(per, q_ids):
        ...
        items.append({"id": ex["id"], "family": ex["family"], "type": q.type, "state": s, "q": qseg, "ids": leaves, "target": target})
...
@torch.no_grad()
def score_items(model, items, roots, max_batch_tokens):
    """Inference path (root KV cache + branches) -> per-item score lists."""
    model.lm.eval()
    out = [None] * len(items)
    for b in micro_batches(items, roots, max_batch_tokens, random.Random(0)):
...
def train(config_path=None, **overrides):
    ...
    evaluate("init (zero-initialized LoRA = the unmodified backbone in tree format)")
    ...
    for epoch in range(cfg["epochs"] if not cfg["max_steps"] else 10**9):
        ...
        for g in range(0, len(batches), cfg["grad_accum"]):
            ...
            for b in group:
                ...
                loss = grouped_loss(model.packed(trees_for(items, train_roots)), items) / nq
                loss.backward()
                step_loss += loss.item()
    ...
    meta = {"config": cfg, "base": {"model": cfg["model_id"], "revision": cfg["revision"]}, "format": format_config(cfg["model_id"]),

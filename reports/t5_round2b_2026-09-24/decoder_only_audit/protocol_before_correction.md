...
Status: training complete; controls and serving optimizations running, 24 September 2026 UTC. Validation selected step 228 (77.59% accuracy, loss 0.379396), and adapter reload reproduced checked scores exactly. The fixed unmerged T5 scores 73.84% on the development benchmark and 76.80% on eval2, with 266.61 ms p50 / 269.38 ms p95 at 2K × 16 × 3 on L40S. This is a substantial quality shortfall versus the existing round-2b report; the complete same-GPU comparison follows.
...
Twenty-six local checks pass (two real-model integration tests excluded from this CPU regression run), including a real tiny T5 decoder, independent-vs-shared scoring, distinct-document isolation, cache storage scaling and gradients through the full encoder/decoder. Full-model FP32 independent/shared/permuted inference differs by at most 0.000014 on 64/1,024-token inputs (`actual_fp32_checks.json`). A one-step eight-question training/reload smoke test passed; it was a wiring check, not an overfitting result. The complete run rechecks the selected adapter after reload.

## Cloud and reproducibility

User authorized GPU provisioning in the conversation. Dedicated `g6e.4xlarge` L40S in Ohio, instance `i-087b024b35cff657b`, launched 06:37:50 UTC. Quoted compute rate $3.00424/hour; 200 GB encrypted gp3, delete on termination. Eight-hour OS shutdown cap, terminate on shutdown, scheduled for 14:38:54 UTC. Virginia had insufficient capacity; no instance was launched there. Its temporary security group/key pair will also be removed. Final wall time, cost estimate and cleanup evidence will be recorded.
...
## Conditional hardware follow-up

After the same-L40S architecture screen, run the retained serving path on one H100 (`p5.4xlarge`, Ohio on-demand price verified at $6.88/hour), subject to available capacity. Keep the same checkpoint, code and request bytes; compare BF16 and supported FP8 where applicable, with complete quality checks. Use a separate `h100/` output directory so hardware results cannot overwrite or masquerade as the same-GPU architecture comparison. Provision only after selecting the path, with a two-hour terminate-on-shutdown cap.

Historical-adapter reproduction in the new bounded-cache harness: all 3,471 development decisions match the historical cross-KV report, retaining 75.37% accuracy; maximum BF16 raw-score difference is 0.10964 (`prior_reproduction.json`). The new training recipe loses 1.53 percentage points on this development set, paired source-cluster 95% CI [−2.72, −0.38] points (`t5_training_recipe_development_comparison.json`). This does not substitute for the task-focused eval2 comparison.
